...
- ✅ Online tables, the two follow-ups from the security review of 2026-09-22, both closed in v1.2.1: the receiver now writes results only for accounts on the roster that game was sent (kept on `finaltable_games.roster`, refusals logged once per delivery), and *hide guest list* is documented as binding the event page's guest list rather than the watch link — the rail is pass-on-able by design on both sides, and every invitee already has the link by email
