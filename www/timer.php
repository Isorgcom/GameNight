<?php
/**
 * The Tournament Timer — the layout-engine display, and the doorway to Classic.
 *
 * DISPLAY-ONLY. State comes from timer_dl.php?action=get_state, the same read
 * path the remote viewer uses, so this page has no write surface at all. A
 * layout is a JSON tree of rows/columns/cells rendered into nested flexbox
 * (see timer_display.js); built-in layouts ship as starting points and the
 * editor (timer_layouts.php) builds custom ones.
 *
 * With ?event_id=N it shows that game's live state (host rights required).
 * Without one it runs on sample data so layouts can be judged without a game
 * in progress.
 *
 * This page was timer_beta.php until v1.1.0, when the layout timer became the
 * Tournament Timer and the old display became Tournament Timer Classic. It
 * holds the /timer.php address because that is the one printed on QR codes,
 * saved in bookmarks and linked from everywhere else in the app - see the
 * dispatch below, which sends the links that belong to Classic onward.
 */
...
// ─── Classic's doorway ───────────────────────────────────────────────────
// Three kinds of link belong to Tournament Timer Classic, and all three are
// forwarded rather than broken: a cast/QR link it printed (?view=remote&key=,
// which must be caught BEFORE this page reads ?key= as its own cast key), the
// explicit ?classic=1 escape hatch, and a game whose Setup switch says
// Classic. timer_classic.php forwards back only in the opposite case
// (use_beta set), so the two can never bounce a request between them.
$_qs = (string)($_SERVER['QUERY_STRING'] ?? '');
if ((isset($_GET['view']) && $_GET['view'] === 'remote') || !empty($_GET['classic'])) {
    header('Location: /timer_classic.php' . ($_qs !== '' ? '?' . $_qs : ''));
    exit;
}
$_want_eid = (int)($_GET['event_id'] ?? 0);
if ($_want_eid > 0 && empty($_GET['embed'])) {
    $_u = $db->prepare('SELECT ts.use_beta FROM timer_state ts
                        JOIN poker_sessions ps ON ps.id = ts.session_id
                        WHERE ps.event_id = ?');
    $_u->execute([$_want_eid]);
    $_row = $_u->fetch();
    // No timer row yet means this game has never opened a timer: Classic owns
    // the code that creates the row from the host's own default, so send it
    // there and let it decide (it forwards back here when the answer is this
    // display).
    if (!$_row || empty($_row['use_beta'])) {
        header('Location: /timer_classic.php' . ($_qs !== '' ? '?' . $_qs : ''));
        exit;
    }
}
...
if ($key_timer) {
    ...
    // Minimal denial pages: this page keeps zero dependencies on partials, so
    // it renders on its own. Wording matches _event_denied.php.
    if ($event_title === false) {
        http_response_code(404);
        exit('<!doctype html><meta charset="utf-8"><title>Game not found</title><body style="background:#0f172a;color:#e2e8f0;font-family:sans-serif;text-align:center;padding-top:4rem"><h1>That game doesn&#8217;t exist</h1><p><a style="color:#60a5fa" href="/timer.php">Open the Tournament Timer with sample data</a></p></body>');
    }
    if (!check_event_access($db, $event_id, $current, $isAdmin)) {
        http_response_code(403);
        exit('<!doctype html><meta charset="utf-8"><title>Game not available</title><body style="background:#0f172a;color:#e2e8f0;font-family:sans-serif;text-align:center;padding-top:4rem"><h1>You can&#8217;t view this game&#8217;s timer</h1><p>Running or viewing a game timer takes host rights.</p><p><a style="color:#60a5fa" href="/timer.php">Open the Tournament Timer with sample data</a></p></body>');
    }
...
<head>
    ...
    <title>Tournament Timer &mdash; <?= htmlspecialchars($site_name) ?></title>
    <link rel="stylesheet" href="/fonts.css?v=<?= htmlspecialchars(APP_VERSION . '.' . (@filemtime(__DIR__ . '/fonts.css') ?: 0)) ?>">
    <link rel="stylesheet" href="/timer_display.css?v=<?= htmlspecialchars(APP_VERSION . '.' . (@filemtime(__DIR__ . '/timer_display.css') ?: 0)) ?>">
</head>
<body>
...
<?php /* Sample/library mode only. A scanned screen never gets the bar (the
         ...
         board is a clean TV face — no picker, no chrome. */ ?>
...
<button type="button" id="tbFsBtn" data-tb="fullscreen" title="Fullscreen" aria-label="Fullscreen">&#9974;</button>
...
<!-- Control tray: shown by timer_display.js only once get_state confirms the
...
<div id="tbControls" hidden>
    ...
    <span class="tb-ctrl-sep"></span>
    <button type="button" data-tb="fullscreen" title="Fullscreen" aria-label="Fullscreen">&#9974;</button>
    ...
    <span class="tb-ctrl-sep"></span>
    <button type="button" class="tb-ctrl-exit" data-tb="exit" title="Leave the timer (back to the game)" aria-label="Leave the timer">Exit</button>
</div>
<?php endif; ?>
...
var TB_ASSET_V  = <?= (int) (@filemtime(__DIR__ . '/timer_display.js') ?: 0) ?>;
...
<script src="/timer_display.js?v=<?= htmlspecialchars(APP_VERSION . '.' . (@filemtime(__DIR__ . '/timer_display.js') ?: 0)) ?>"></script>
</body>
</html>
