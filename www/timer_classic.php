<?php
/* ============================================================================
 * TOURNAMENT TIMER CLASSIC — www/timer_classic.php
 *
 * The original full-screen blind clock. It was www/timer.php until v1.1.0,
 * when the layout-engine display took the Tournament Timer name and the
 * /timer.php address; every link ever printed for this page still works,
 * because timer.php forwards ?view=remote, ?classic=1 and any game set to
 * Classic straight back here.
 * ============================================================================
 */
/* ============================================================================
...
// ─── §1.2  Host mode ────────────────────────────────────────────
    ...
    if ($event_id) {
        ...
        // This game's Setup switch says the Tournament Timer, so hand it over
        // (this page owns creating the timer row, which is why the handover
        // happens here rather than at the door). ?classic=1 keeps the request
        // on Classic. timer.php forwards to this page only in the opposite
        // case, so the two cannot bounce a request between them.
        if (!empty($timer['use_beta']) && empty($_GET['classic'])) {
            header('Location: /timer.php?event_id=' . (int)$event_id);
            exit;
        }
...
<head>
    ...
    <title>Tournament Timer Classic &mdash; <?= htmlspecialchars($site_name) ?></title>
    ...
    <link rel="stylesheet" href="/timer_classic.css?v=<?= htmlspecialchars(APP_VERSION . '.' . (@filemtime(__DIR__ . '/timer_classic.css') ?: 0)) ?>">
</head>
...
<script src="/timer_classic.js?v=<?= htmlspecialchars(APP_VERSION . '.' . (@filemtime(__DIR__ . '/timer_classic.js') ?: 0)) ?>"></script>
