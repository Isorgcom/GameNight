 ...
 *
 * v1 is tournaments only; cash games and session creation 302 to the console.
 * Finish game deliberately stays on the console too — it mints entry tickets
 * and its reversal can be blocked, a decision for the payout ladder, not a
 * thumb reaching for KO.
 */
...
// Read-only bootstrap. No session yet, or a cash game → the console owns those
// flows; send the host there rather than half-rendering.
$sq = $db->prepare('SELECT * FROM poker_sessions WHERE event_id = ?');
$sq->execute([$event_id]);
$session = $sq->fetch();
if (!$session || ($session['game_type'] ?? '') !== 'tournament') {
...
<head>
    ...
    .gd-foot button { pointer-events:auto; display:block; width:100%; max-width:560px; margin:0 auto;
        border:none; border-radius:12px; min-height:50px; font-weight:800; font-size:.95rem;
...
<div class="gd-wrap">
    ...
    <div class="gd-toolbar">
        ...
        <div class="pk-seg" id="gdViewSeg">
            ...
            <button type="button" data-view="out" data-act="gdSetView" data-a1="out">Out</button>
...
<div class="gd-foot"><button type="button" data-act="gdAddWalkin">+ Walk-in</button></div>
