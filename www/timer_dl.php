...
function compute_live_state($db, $timer) {
    ...
    if ($running && $timer['updated_at']) {
        ...
        if ($remaining <= 0 && $timer['preset_id']) {
            ...
            $db->prepare("UPDATE timer_state SET current_level = ?, time_remaining_seconds = ?, is_running = ?, updated_at = datetime('now') WHERE session_id = ?")
                ->execute([$level, max(0, $remaining), $running, $session_id]);
        }
    }
...
// ─── Helper: resolve session from key or session_id ───────
...
if ($action === 'command') {
    ...
    // Resolve timer from key or session_id
    if (!empty($_POST['key'])) {
        $timer = resolve_timer($db, $_POST['key']);
    } elseif (!empty($_POST['session_id'])) {
        $timer = resolve_timer($db, null, (int)$_POST['session_id']);
    }
    ...
    $db->prepare("UPDATE timer_state SET is_running = ?, current_level = ?, time_remaining_seconds = ?, updated_at = datetime('now') WHERE session_id = ?")
        ->execute([$running, $level, $remaining, $session_id]);
...
if ($action === 'load_preset') {
    ...
    $db->prepare("UPDATE timer_state SET preset_id = ?, current_level = 1, time_remaining_seconds = ?, is_running = 0, updated_at = datetime('now') WHERE session_id = ?")
        ->execute([$preset_id, $duration, $session_id]);
...
if ($action === 'update_levels') {
    ...
    $ts = $db->prepare('SELECT preset_id FROM timer_state WHERE session_id = ?');
    $ts->execute([$session_id]);
    $timer = $ts->fetch();
    if (!$timer || !$timer['preset_id']) {
    ...
    if ($presetRow && (int)$presetRow['is_default']) {
        ...
        $db->prepare("UPDATE timer_state SET preset_id = ?, updated_at = datetime('now') WHERE session_id = ?")->execute([$preset_id, $session_id]);
    }
...
if ($action === 'update_sounds') {
    ...
    $db->prepare("UPDATE timer_state SET warning_seconds = ?, alarm_sound = ?, warning_sound = ? WHERE session_id = ?")
        ->execute([$warning_seconds, $alarm_sound, $warning_sound, $session_id]);
