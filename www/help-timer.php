...
<div class="help-step" id="two-timers">
    ...
    <div class="help-step" id="building">
        ...
        <p><strong>Fonts:</strong> every text cell has a <strong>Font</strong> setting in the Inspector and the right-click menu, each choice previewed in the dropdown itself. Eight are system looks (Serif, Monospace, Condensed, Wide, Heavy, Impact, Script, Comic) built from faces every device already has; the rest ship with the site &mdash; scoreboard and poster faces (Bebas Neue, Oswald, Anton, Orbitron), casino serifs (Cinzel, Playfair), a western saloon (Rye), a neon sign (Monoton), a brush script (Lobster), and <strong>Digital clock</strong>, a real 7-segment face made for <code>&lt;clock&gt;</code> and blinds cells (digits only &mdash; it is not for sentences). Bundled fonts load from this site, never a third party, and every choice has a same-shape fallback while it loads. A <strong>shared style</strong> can carry a font too, so one setting gives a whole design its typeface.</p>
