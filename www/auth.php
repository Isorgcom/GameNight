...
function current_user(): ?array {
    ...
    $stmt = get_db()->prepare('SELECT id, username, email, role, last_login, must_change_password, my_events_past_days, my_events_future_days, timezone, last_poker_default, mfa_enabled, mfa_method, mfa_offer_dismissed FROM users WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch() ?: null;
