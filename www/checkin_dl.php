...
// Helper: verify session access via player_id
...
// Helper: calculate pool stats for a session
function calc_pool($db, $session_id) {
    ...
    if ($s['game_type'] === 'cash') {
        // Cash game: pool is sum of all cash_in values
...
// Helper: sync invitees from event_invites into poker_players
function sync_invitees($db, $session_id, $event_id) {
    ...
    foreach ($invites->fetchAll() as $inv) {
        if (!in_array(strtolower($inv['username']), $existingNames)) {
            // New invitee — add them
            $pIns->execute([$session_id, $inv['user_id'], $inv['username'], $inv['rsvp']]);
        } else {
            // Existing — update RSVP from event_invites (unless overridden locally)
            $pUpd->execute([$inv['rsvp'], $session_id, $inv['username']]);
        }
    }
}

// Helper: get all players for a session
...
// Helper: get payouts for a session
