...
- **Five pre-push sweeps** (parse errors, affected browser suites, the double-dispatch sweep, known-bad escaping, and the declarative-markup check). See `SECURITY.md` for the commands and why each exists. Run them before any push touching more than one PHP file, and again against production after a deploy. The double-dispatch sweep needs an admin session or it silently skips every admin page.
